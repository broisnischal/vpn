...
# AWS Deployment Script for OMail VPN Server
# This script sets up the VPN server on an AWS EC2 instance
...
echo -e "${GREEN}=== OMail VPN - AWS Deployment ===${NC}"
echo ""
...
# Get network interface (usually eth0 or ens5)
...
if [ ! -d "/opt/omail" ]; then
    ...
    echo "  or"
    echo "  scp -r omail/ root@SERVER:/opt/"
    exit 1
fi
...
if docker-compose ps | grep -q "Up"; then
    ...
    echo "  1. Configure AWS Security Group to allow UDP 51820"
